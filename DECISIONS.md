...
A **virtual joystick** (a translucent thumbstick pinned bottom-right) plus swipe-to-move on the canvas, and tap-to-restart after a game over. Dragging the knob snaps the direction to the nearest cardinal axis (the game is grid-based), moves once, and auto-repeats while held like key auto-repeat; releasing snaps the knob back and stops. It floats over the board rather than reserving vertical space.

### Floating controls & pick-up hint
The joystick is a **translucent overlay on the board** (racing-game style), so the play area keeps the full height — the phone view now shows all 15 rows. It turns opaque while in use so it stays findable without hiding the game. Because off-screen pick-ups are easy to lose in the camera view, a **pulsing chevron** beside the player points to the nearest collectible — but only while *none* of them are on screen (green pick-ups, the blue bonus, and the yellow bonus all count; as soon as any of them is visible the hint hides). It has a dark halo so it reads on both the light Classic and dark Space boards.
