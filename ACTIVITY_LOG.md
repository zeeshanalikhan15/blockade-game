...
28. **Floated the d-pad over the board** — the on-screen controls are now a translucent overlay on the board's lower-left corner (racing-game style) instead of reserving a strip below it, so the phone view now shows the full board height (15 rows) instead of cutting it short. It turns opaque on press.
29. **Added a pick-up direction hint** — in the phone camera view, a small pulsing chevron beside the player points toward the nearest pick-up ("astronaut"), since off-screen pick-ups are otherwise easy to lose.
